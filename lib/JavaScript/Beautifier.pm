...
my (
  $token_text, $last_type,        $last_text,     $last_last_text,
  $last_word,  $current_mode,     $indent_string, $parser_pos,
  $in_case,    $prefix,           $token_type,    $do_block_just_closed,
  $var_line,   $var_line_tainted, $if_line_flag,  $wanted_newline,
  $just_added_newline
);
...
sub js_beautify {
  ...
  foreach my $line (@lines) {
    ...
    if ( $line =~ /beautify-off/ ) {
      push @to_process, "//SKIPPED_BLOCK_$block_num";
      push @block,      $line;
      if ($skipping){
        die("Unbalanced beautify-on/beautify-off lines");
      }
      $skipping = 1;
    } elsif ( $line =~ /beautify-on/ ) {
      push @block, $line;
      push @skipped_blocks, [@block];
      @block    = ();
      $skipping = 0;
      $block_num++;
    } else {
  ...
  die("Unbalanced beautify-on/beautify-off lines") if $skipping;

  $js_source_code = join( '', @to_process );
...
# some formatting depends on that.
  ...
  while (1) {
	...
	unindent();
      ...
      } elsif ( $last_type eq 'TK_SEMICOLON'
      ...
      if ( $last_type ne 'TK_END_BLOCK'
        && ( grep { lc($token_text) eq $_ } ( 'else', 'catch', 'finally' ) ) ) {
        print_newline();
      } elsif ( (
          grep {
            $token_text eq $_
        ...
        if ( $last_text eq 'else' ) {
          ...
          print_space();
        } elsif ( ( $last_type eq 'TK_START_EXPR' || $last_text eq '=' || $last_text eq ',' )
          && $token_text eq 'function' ) {
  ...
  # Re-introduce skipped blocks verbatim to output
  foreach my $block_num ( 0 .. $#skipped_blocks ) {
    my $block = join( '', @{ $skipped_blocks[$block_num] } );
    $output =~ s!$opt_indent_character*//SKIPPED_BLOCK_$block_num!$block!m;
  }
...
sub get_next_token {
  ...
  if ( $c eq '/' ) {
    ...
    if ( $input[$parser_pos] eq '*' ) {
      ...
      if ( $parser_pos < scalar @input ) {
        while (
          not( $input[$parser_pos] eq '*'
            && $input[ $parser_pos + 1 ]
            && $input[ $parser_pos + 1 ] eq '/' )
  ...
  if ( $c eq '<'
    && $parser_pos + 2 < scalar @input
    && join( '', @input[ $parser_pos - 1 .. $parser_pos + 2 ] ) eq '<!--' ) {
    $parser_pos += 3;
    return [ '<!--', 'TK_COMMENT' ];
  }
  if ( $c eq '-'
    && $parser_pos + 1 < scalar @input
    && join( '', @input[ $parser_pos - 1 .. $parser_pos + 1 ] ) eq '-->' ) {
    $parser_pos += 2;
    if ($wanted_newline) {
      print_newline();
    }
